...
// slides.templ — página de geração de slides.

//

// A página é servida em /slides e, diferentemente de antes (HTMX), agora

// renderiza a grade de hinos diretamente no HTML. Ao enviar o formulário com

// a coletânea selecionada (?codigo=), a própria página é re-carregada com a

// grade já preenchida para aquela coletânea.
...
func slidesContent(coletaneas []models.Coletanea, hinos []models.Hino, selecionada string) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		ctx = templ.ClearChildren(ctx)
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 1, "<h1 class=\"mb-6 text-2xl font-bold text-slate-800\">Geração de Slides</h1><form method=\"get\" action=\"/slides\" class=\"mb-6\"><label for=\"coletanea\" class=\"mb-2 block text-sm font-medium text-slate-700\">Coletânea</label><div class=\"flex max-w-md items-center gap-3\"><select id=\"coletanea\" name=\"codigo\" class=\"w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none\"><option value=\"\"")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		if selecionada == "" {
		...
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 4, ">Selecione uma coletânea</option> ")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		...
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 10, "</select> <button type=\"submit\" class=\"rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700\">Listar</button></div></form>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		if selecionada != "" {
